...
class Holder extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {activeTab: (Array.isArray(props.children) ? props.children[0].props.label : props.children.props.label)};
  }
      ...
      <div style={{display: "flex", flexDirection: "row"}}>
        <div style={{alignSelf: "flex-start"}}>
          <MenuWrapper style={{flexDirection: "column", display: "inline-block"}}>
            {
              tabIDs.map(tabId => {
                return <TabButton key={tabId} clickEvent={tabId => this.setState({activeTab: tabId}) } isActive={tabId === this.state.activeTab} tabId={tabId} />
              })
            }
          </MenuWrapper>
        </div>
        <Container>
          {children.find(child => child.props.label === this.state.activeTab)}
        </Container>
      </div>
    )
  }
