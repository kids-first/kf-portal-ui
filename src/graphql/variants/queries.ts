...
export const SEARCH_VARIANT_SOMATIC_QUERY = gql`
  query searchVariantSomatic(
    $sqon: JSON
    $first: Int
    $offset: Int
    $sort: [Sort]
...
export const GET_VARIANT_SOMATIC_COUNT = gql`
  query getVariantsSomaticCount($sqon: JSON) {
    variants_somatic {
      hits(filters: $sqon) {
        total
      }
...
export const GET_VARIANT_SOMATIC_ENTITY = gql`
  query getVariantSomaticEntity($sqon: JSON) {
    variants_somatic {
      hits(filters: $sqon) {
        edges {
          node {
