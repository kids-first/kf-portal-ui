...
describe('Page Data Exploration (Data Files) - Vérifier les informations affichées', () => {
  ...
  it('Tableau', () => {
    ...
    cy.get('tr[data-row-key="GF_6DVS70V9"] [class*="ant-table-cell"]').eq(11).contains('-').should('exist');
    ...
    cy.get('tr[data-row-key="dr-2ahd9kpwqk"] [class*="ant-table-cell"]').eq(21).contains('Functional').should('exist');
    cy.get('tr[data-row-key="dr-2ahd9kpwqk"] [class*="ant-table-cell"]').eq(22).contains('CTSPINE').should('exist');
    cy.get('tr[data-row-key="dr-2ahd9kpwqk"] [class*="ant-table-cell"]').eq(23).contains('1.3717760004328263').should('exist');
    cy.get('tr[data-row-key="dr-2ahd9kpwqk"] [class*="ant-table-cell"]').eq(24).contains('Canon').should('exist');
    cy.get('tr[data-row-key="dr-2ahd9kpwqk"] [class*="ant-table-cell"]').eq(25).contains('Avanto_fit').should('exist');
    cy.get('tr[data-row-key="dr-2ahd9kpwqk"] [class*="ant-table-cell"]').eq(26).contains('de-pea26nptxz').should('exist');
  });
});
