...
describe('Page Profile Settings - Valider les liens disponibles', () => {
  ...
  it('Action du toggle de la section Identification', () => {
    cy.get('input[id="first_name"]').clear({force: true}).type('Cypress', {force: true}).should('have.attr', 'value', 'Cypress');
    ...
    cy.typeAndIntercept('[class*="Box_filterContentWrapper"]', 'Cypress', 'GET', '**/user/search?**', 1);
    cy.get('[class*="CommunityPage_usersListWrapper"]').contains('1 Result').should('exist');
    
    cy.visitProfileSettingsPage();
    cy.get('[class*="Gridcard_fuiCardWrapper"]').eq(0).find('[class="ant-card-head"] button[class*="ant-switch"]').clickAndWait({force: true});
    cy.visitCommunityPage();
    cy.typeAndIntercept('[class*="Box_filterContentWrapper"]', 'Cypress', 'GET', '**/user/search?**', 1);
    cy.get('[class*="CommunityPage_usersListWrapper"]').contains('No Public Members').should('exist');
  });
